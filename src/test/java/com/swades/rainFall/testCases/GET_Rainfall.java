...
public class GET_Rainfall {

	static GenerateToken tg = new GenerateToken();
	static String token = tg.getAccessToken();

	@Test
	public static void get_Rainfall() {

		Response response =

				given().header("Authorization", "Bearer " + token).contentType(ContentType.JSON)
				.queryParams("stationId", "1")
				.queryParams("month", "9").queryParams("year", "1988")

						.log().all().get(SwadesAPIsEndPoints.GET_rainfall);
