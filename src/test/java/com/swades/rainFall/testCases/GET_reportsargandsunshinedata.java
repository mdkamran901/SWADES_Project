...
public class GET_reportsargandsunshinedata {
	static GenerateToken tg = new GenerateToken();
	static String token = tg.getAccessToken();

	@Test
	public static void get_reportsargandsunshinedata() {

		Response response =

				given().header("Authorization", "Bearer " + token).contentType(ContentType.JSON)
				.queryParams("station_id", "45")
				.queryParams("fromDate", "2015-12-28").queryParams("toDate", "2015-12-31")

						.log().all().get(SwadesAPIsEndPoints.GET_reportsargandsunshinedata);
