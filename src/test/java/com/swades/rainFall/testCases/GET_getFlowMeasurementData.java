...
public class GET_getFlowMeasurementData {
	static GenerateToken tg = new GenerateToken();
	static String token = tg.getAccessToken();

	@Test
	public static void getFlowMeasurementData() {

		Response response =

				given().header("Authorization", "Bearer " + token).contentType(ContentType.JSON)
				.queryParams("stationId", "138").queryParams("day", "22")
				.queryParams("month", "6").queryParams("year", "2007")

						.log().all().get(SwadesAPIsEndPoints.GET_getFlowMeasurementData);
