...
public class GET_reportsclimaticReport {
	static GenerateToken tg = new GenerateToken();
	static String token = tg.getAccessToken();

	@Test
	public static void get_reportsclimaticReport() {

		Response response =

				given().header("Authorization", "Bearer " + token).contentType(ContentType.JSON)
				.queryParams("station_ids", "14").queryParams("station_ids", "15")
				.queryParams("flag", "4").queryParams("frequency", "2")
				.queryParams("fromDate", "1999-10-08").queryParams("toDate", "2000-10-08")

						.log().all().get(SwadesAPIsEndPoints.GET_reportsclimaticReport);
