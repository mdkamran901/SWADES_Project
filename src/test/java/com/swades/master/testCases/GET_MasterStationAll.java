...
public class GET_MasterStationAll {

	static GenerateToken tg = new GenerateToken();
	static String token = tg.getAccessToken();

	@Test
	public static void get_DownloadMasterDetails() {

		Response response =

				given().header("Authorization", "Bearer " + token).contentType(ContentType.JSON)
						.header("userName", "QATest").queryParams("station_id", "45")

						.log().all().get(SwadesAPIsEndPoints.GET_MasterStationall);
