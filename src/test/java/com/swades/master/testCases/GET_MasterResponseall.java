...
public class GET_MasterResponseall {
	static GenerateToken tg = new GenerateToken();
	static String token = tg.getAccessToken();

	@Test
	public static void get_MasterResponseall() {

		Response response =

				given().header("Authorization", "Bearer " + token).contentType(ContentType.JSON)
						.header("userName", "QATest").queryParams("Flag", "99")

						.log().all().get(SwadesAPIsEndPoints.GET_MasterResponseall);
