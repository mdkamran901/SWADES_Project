...
public class GET_StationWiseSeriesData {

	static GenerateToken tg = new GenerateToken();
	static String token = tg.getAccessToken();

	@Test
	public static void StationWiseSeriesData() {

		Response response =

				given().header("Authorization", "Bearer " + token).contentType(ContentType.JSON)
						.header("userName", "QATest").queryParams("station_id", "12")

						.log().all().get(SwadesAPIsEndPoints.GET_stationWiseSeriesData);
