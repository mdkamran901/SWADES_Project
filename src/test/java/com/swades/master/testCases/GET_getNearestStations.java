...
public class GET_getNearestStations {
	static GenerateToken tg = new GenerateToken();
	static String token = tg.getAccessToken();

	@Test
	public static void getNearestStations() {

		Response response =

				given().header("Authorization", "Bearer " + token).contentType(ContentType.JSON)
						.header("userName", "QATest").pathParam("stationId", "85")

						.log().all().get(SwadesAPIsEndPoints.GET_getNearestStations);
